...
For example, with the above flag turned on when generating from javascript to swift, an enum with keys `foo_bar` and `FOO_BAR` will generate into an enum with a single key, `fooBar`, since standard swift styles camelCase keys.
